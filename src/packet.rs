...
impl PacketType {
    fn from_i32(value: i32, response: bool) -> Self {
        match value {
            3 => PacketType::Auth,
            2 if response => PacketType::AuthResponse,
...
impl Packet {
    pub(crate) fn new(id: i32, packet_type: PacketType, payload: String) -> Self {
    ...
    pub(crate) async fn write_to_io<T: Unpin + AsyncWrite>(&self, io: &mut T) -> io::Result<()> {
        ...
        writer.write_i32_le(self.packet_type.to_i32()).await?;
    ...
    pub(crate) async fn read_from_io<T: Unpin + AsyncRead>(io: &mut T) -> io::Result<Self> {
        ...
        let packet_type = PacketType::from_i32(reader.read_i32_le().await?, true);
        ...
        let payload = match payload {
            ...
            Err(_) => Err(io::Error::new(
                ...
                "Invalid UTF-8 in packet payload",
            ))?,
        };
